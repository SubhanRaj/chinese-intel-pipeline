...
export default function IntelViewer({ briefings, articles }: Props) {
	...
	const sidebar = (
		<aside className="w-80 shrink-0 flex flex-col border-r border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 print:hidden">
			{/* Header */}
			<div className="px-5 py-5 border-b border-slate-200 dark:border-slate-800 flex items-start justify-between">
				<div>
					...
					<p className="text-sm text-slate-400 mt-1">Daily briefings · CST</p>
				</div>
				<button
					onClick={() => setDark((d) => !d)}
					className="mt-1 p-2 rounded-md text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
			...
			<nav className="flex-1 overflow-y-auto py-2 px-2">
				{briefings.length === 0 ? (
					<p className="text-sm text-slate-400 text-center mt-8 px-4 leading-relaxed">
					...
					briefings.map((b) => {
						...
						return (
							<button
								key={b.id}
								onClick={() => { setSelectedId(b.id); setShowRaw(false); setDrawerArticle(null); }}
								className={[
							...
							>
								<IconCalendar size={16} className="shrink-0 opacity-60" />
								<span className="flex-1 text-sm font-mono font-medium tracking-wide">
									{b.trackingDate}
								</span>
								{isActive && <IconChevronRight size={14} className="text-red-500 shrink-0" />}
							</button>
						);
					})
				)}
			...
			<div className="flex h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-hidden">
				...
				<main className="flex-1 flex items-center justify-center bg-slate-50 dark:bg-slate-950">
					<div className="text-center max-w-sm px-6">
						...
						<div className="mt-6 inline-flex items-center gap-2 text-sm text-slate-400 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2">
			...
			<main className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-950 print:overflow-visible print:h-auto">
				{selected === null ? (
					<div className="flex flex-col items-center justify-center h-full text-slate-400 gap-3">
						...
						<p className="text-base">Select a briefing date from the sidebar.</p>
					</div>
				) : (
					<div className="max-w-3xl mx-auto px-10 py-10">
						...
						<header className="mb-8 pb-6 border-b border-slate-200 dark:border-slate-800">
							...
							<h2 className="text-4xl font-bold text-slate-900 dark:text-slate-100 tracking-tight font-mono">
							...
							<div className="flex items-center justify-between mt-4 flex-wrap gap-3">
								...
								<div className="flex items-center gap-2 print:hidden">
									{hasMarkdown && (
										...
										>
											{showRaw ? (
												<><IconLanguage size={15} /> Show English</>
											) : (
												<><IconSourceCode size={15} /> View Source 中文</>
											)}
										</button>
									)}
						...
						{showRaw && hasMarkdown ? (
							<div className="rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50/50 dark:bg-slate-900/60 p-6">
								...
								<pre className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-wrap break-words font-mono">
	...
	return (
		<Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-none">
			...
			<CardContent className="pt-0 pb-4 space-y-4">
				...
				{/* Chinese source accordion */}
				{article.fullText && (
					<Accordion>
						<AccordionItem value="full-text" className="border-slate-200 dark:border-slate-700">
							<AccordionTrigger className="text-sm text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 py-2">
								View source text (Chinese)
							</AccordionTrigger>
							<AccordionContent>
								<pre className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed whitespace-pre-wrap break-words font-mono bg-slate-50 dark:bg-slate-950 rounded-lg p-4 mt-2">
									{article.fullText}
								</pre>
							</AccordionContent>
						</AccordionItem>
				...
				{/* Action row — inline, no CardFooter */}
				<div className="flex items-center justify-between pt-1 border-t border-slate-100 dark:border-slate-800">
					<div className="flex items-center gap-3">
						...
						{article.url && (
							<a
								...
								className="inline-flex items-center gap-1.5 text-sm text-blue-600 dark:text-blue-400 hover:underline underline-offset-2"
							>
					...
					<div className="flex items-center gap-1 print:hidden">
						<Button
							...
							title={article.isPreserved ? 'Remove from preserved' : 'Preserve'}
						>
						...
						<Button
							...
							className="h-8 px-3 text-sm gap-1.5 text-slate-400 hover:text-red-600 dark:hover:text-red-400"
							title="Delete"
						>
							<IconTrash size={15} />
							Delete
...
function ArticleDrawer({ article, onClose }: DrawerProps) {
	...
	return (
		<>
			...
			<div
				className={[
					'fixed top-0 right-0 z-50 h-full w-[48%] min-w-[420px] bg-white dark:bg-slate-900 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out',
			...
			>
				...
				<div className="flex items-center justify-between px-7 py-5 border-b border-slate-200 dark:border-slate-800 shrink-0">
					<div>
						<p className="text-xs font-bold tracking-widest uppercase text-red-500 mb-0.5">
							Full Article
						</p>
						<p className="text-sm text-slate-400">English analysis + Chinese source</p>
					</div>
					<button
						onClick={onClose}
				...
				<div className="flex-1 overflow-y-auto px-7 py-6 space-y-6">
					{article && (
						<>
							{/* Title */}
							<div>
								<h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 leading-snug mb-3">
									{article.title}
								</h2>
								{article.url && (
									<a
							...
							<div>
								...
								<p className="text-base text-slate-700 dark:text-slate-200 leading-relaxed">
									{article.summary?.replace(/\[HIGH\]/g, '').trim() ?? 'No analysis available.'}
								</p>
							</div>
